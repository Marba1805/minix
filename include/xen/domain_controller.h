 ...
 *
 * Minor changes made for minix datatypes.
 *
 ...
 */
typedef struct {
  u8_t type;		/*  0: echoed in response */
  u8_t subtype;		/*  1: echoed in response */
  u8_t id;		/*  2: echoed in response */
  u8_t length;		/*  3: number of bytes in 'msg' */
  u8_t msg[60];		/*  4: type-specific message data */
} PACKED control_msg_t;		/* 64 bytes */

#define CONTROL_RING_SIZE 8
typedef u32_t CONTROL_RING_IDX;
...
#define blkif_vdev_t   u16_t
#define blkif_pdev_t   u32_t
#define blkif_sector_t u64_t
...
typedef struct {
  u32_t handle;		/*  0 */
  u32_t status;		/*  4 */
  u16_t evtchn;		/*  8: (only if status == BLKIF_INTERFACE_STATUS_CONNECTED). */
...
typedef struct {
  /* IN */
  u32_t status;		/*  0: BLKIF_DRIVER_STATUS_??? */
  /* OUT */
...
typedef struct {
  ...
  u16_t __pad;
  u32_t blkif_handle;	/*  4: Domain-specific interface handle.   */
  /* OUT */
  u32_t status;		/*  8 */
...
typedef struct {
  ...
  MEMORY_PADDING;
  u32_t evtchn;		/* 16: Event channel for notifications.    */
  /* OUT */
  u32_t status;		/* 20 */
...
typedef struct {
  ...
  u16_t readonly;		/* 10: Non-zero -> VBD isn't writable.     */
  /* OUT */
  u32_t status;		/* 12 */
...
typedef struct {
  u32_t handle;		/*  0 */
  u32_t status;		/*  4 */
  u16_t evtchn;		/*  8: status == NETIF_INTERFACE_STATUS_CONNECTED */
  u8_t mac[6];		/* 10: status == NETIF_INTERFACE_STATUS_CONNECTED */
...
typedef struct {
  /* IN */
  u32_t status;		/*  0: NETIF_DRIVER_STATUS_??? */
  /* OUT */
  /* Driver should query interfaces [0..max_handle]. */
  u32_t max_handle;	/*  4 */
...
typedef struct {
  ...
  domid_t domid;		/*  0: Domain attached to new interface.   */
  u16_t __pad0;		/*  2 */
  u32_t netif_handle;	/*  4: Domain-specific interface handle.   */
  u8_t mac[6];		/*  8 */
  u16_t __pad1;		/* 14 */
  /* OUT */
  u32_t status;		/* 16 */
...
typedef struct {
  ...
  MEMORY_PADDING;
  u16_t evtchn;		/* 24: Event channel for notifications.    */
  u16_t __pad1;		/* 26 */
  /* OUT */
  u32_t status;		/* 28 */
...
typedef struct {
  /* OUT */
  u32_t target;		/* 0: Target memory reservation in pages.       */
  /* IN  */
  u32_t status;		/* 4: Return code indicates success or failure. */
