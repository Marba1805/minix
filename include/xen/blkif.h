...
typedef struct {
  u8_t operation;		/*  0: BLKIF_OP_???                         */
  u8_t nr_segments;	/*  1: number of segments                   */
...
typedef struct {
  ...
  u8_t operation;		/* copied from request */
  i16_t status;		/* BLKIF_RSP_???       */
...
typedef struct {
  ...
  u16_t info;		/* 10: Device type and flags (VDISK_*).     */
